...
fn test_dense_matmul_ijk_gcn(base_path: &PathBuf) {
    ...
    let bk_rdscanner = UncompressedCrdRdScan::new(bk_data, b_shape[1]);
    ...
    let x0_seg_filename = base_path.join("tensor_X_mode_0_seg_out");
    let x0_crd_filename = base_path.join("tensor_X_mode_0_crd_out");
    let x1_seg_filename = base_path.join("tensor_X_mode_1_seg_out");
    let x1_crd_filename = base_path.join("tensor_X_mode_1_crd_out");
    let x_vals_filename = base_path.join("tensor_X_mode_vals_out");
    write_output::<VT>(&x_vals_filename, x_val.clone());
    write_output::<u32>(&x0_seg_filename, x0_seg.clone());
    write_output::<u32>(&x0_crd_filename, x0_crd.clone());
    write_output::<u32>(&x1_seg_filename, x1_seg.clone());
    write_output::<u32>(&x1_crd_filename, x1_crd.clone());
...
#[test]
fn test_dense_matmul_ijk_looper() {
    let filename = env::current_dir().unwrap().join("tests/dense_matmul.toml");
    ...
    for i in formatted_dir.iter() {
        ...
        test_dense_matmul_ijk_gcn(&path);
    }
}
