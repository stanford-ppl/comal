...
fn test_fixed_multihead_attention() {
    ...
    let qk_rdscanner = CompressedCrdRdScan::new(qk_data, q1_seg.clone().clone(), q1_crd.clone());
    ...
    let mut broadcast100 = BroadcastContext::new(intersectl_out_ref1_receiver);
    let mut broadcast101 = BroadcastContext::new(intersectl_out_ref2_receiver);
    let mut broadcast102 = BroadcastContext::new(intersectl_out_crd_receiver);
    for _ in 0..par_factor {
        let (chunk_qk_ref_sender, chunk_qk_ref_receiver) = parent.bounded(chan_size);
        let (intersectl_ref1_sender, intersectl_ref1_receiver) = parent.bounded(chan_size);
        let (intersectl_ref2_sender, intersectl_ref2_receiver) = parent.bounded(chan_size);
        // let (intersectl_crd_sender, intersectl_crd_receiver) = parent.bounded(chan_size);
        ...
        broadcast100.add_target(intersectl_ref1_sender);
        broadcast101.add_target(intersectl_ref2_sender);
        // broadcast102.add_target(intersectl_crd_sender);
        broadcast102.add_target(bc_intersectl_out_crd_sender);
        broadcast102.add_target(bc1_intersectl_out_crd_sender);
        ...
        // let (bc1_intersectl_out_crd_sender, bc1_intersectl_out_crd_receiver) =
        //     parent.bounded(chan_size);
        // let mut broadcast17 = BroadcastContext::new(intersectl_crd_receiver);
        ...
        let vm_data = RdScanData::<u32, u32> {
            in_ref: intersectl_ref1_receiver,
        ...
        let km_data = RdScanData::<u32, u32> {
            in_ref: intersectl_ref2_receiver,
    ...
    parent.add_child(broadcast100);
    parent.add_child(broadcast101);
    parent.add_child(broadcast102);
