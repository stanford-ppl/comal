    ...
    for operation in comal_graph.graph.unwrap().operators {
        match operation.op.expect("Error processing") {
            ...
            Op::Joiner(op) => {
                ...
                // Use wildcard type Token<_, ST> to accept both CT (coordinates) and VT16 (Tensor blocks)
                let mut in_crds = Vec::new();
                let mut in_refs: Vec<Box<dyn RecvAdapter<Token<_, ST>> + Send + Sync>> = Vec::new();
                let mut out_refs: Vec<Box<dyn SendAdapter<Token<_, ST>> + Send + Sync>> = Vec::new();
                op.input_pairs.iter().for_each(|pair| {
                    let pair_crd = crdmap.get_receiver(get_crd_id(&pair.crd), builder);
                    match pair.in_ref.clone().unwrap().stream.as_ref().unwrap() {
                        joiner::payload::Stream::RefStream(ref_stream) => {
            ...
            Op::Alu(op) => {
                ...
                if in_val_ids.len() == 2 {
                    ...
                    let (latency, ii, binary_func): (usize, usize, fn(VT16, VT16) -> VT16) = match op.stages[0].op() {
                        ...
                        _ => todo!("Unsupported binary op for block sparse: {:?}", op.stages[0].op()),
                    };
                    builder.add_child(Binary::new(
                        val_receiver1,
                        val_receiver2,
                    ...
                    match op.stages[0].op() {
                        ...
                        alu::AluOp::Exp => {
                            let unary_func = |val: VT16| -> VT16 {
                                Tensor::new(val.data.mapv(|x| x.exp()).to_owned())
                            };
                            builder.add_child(Unary::new(
                                val_receiver1,
                                out_val_sender,
                        ...
                        alu::AluOp::Rsqrt => {
                            let unary_func = |val: VT16| -> VT16 {
                                Tensor::new(val.data.mapv(|x| 1.0 / x.sqrt()).to_owned())
                            };
                            builder.add_child(Unary::new(
                                val_receiver1,
                                out_val_sender,
                        ...
                        alu::AluOp::Sigmoid => {
                            let unary_func = |val: VT16| -> VT16 {
                                Tensor::new(val.data.mapv(|x| 1.0 / (1.0 + (-x).exp())).to_owned())
                            };
                            builder.add_child(Unary::new(
                                val_receiver1,
                                out_val_sender,
                        ...
                        _ => todo!("Unsupported unary op for block sparse: {:?}", op.stages[0].op()),
                    }
                }
            }
            Op::Reduce(op) => {
...
pub fn build_from_proto_block32<'a>(
    ...
    for operation in comal_graph.graph.unwrap().operators {
        match operation.op.expect("Error processing") {
            ...
            Op::Joiner(op) => {
                // Use wildcard type Token<_, ST> to accept both CT (coordinates) and VT32 (Tensor blocks)
                let mut in_crds = Vec::new();
                let mut in_refs: Vec<Box<dyn RecvAdapter<Token<_, ST>> + Send + Sync>> = Vec::new();
                let mut out_refs: Vec<Box<dyn SendAdapter<Token<_, ST>> + Send + Sync>> = Vec::new();
                op.input_pairs.iter().for_each(|pair| {
            ...
            Op::Alu(op) => {
                ...
                if in_val_ids.len() == 2 {
                    ...
                    let (latency, ii, binary_func): (usize, usize, fn(VT32, VT32) -> VT32) = match op.stages[0].op() {
                        ...
                        alu::AluOp::Div => (N * N, 1, |val1: VT32, val2: VT32| -> VT32 { val1 / val2 }),
                        _ => todo!("Unsupported binary op for block sparse: {:?}", op.stages[0].op()),
                    };
                    ...
                    match op.stages[0].op() {
                        ...
                        alu::AluOp::Scalardiv => { let scalar: f32 = op.scalar as f32; builder.add_child(Unary::new(val_receiver1, out_val_sender, move |val: VT32| -> VT32 { Tensor::new(val.data.map(|x| x / scalar).to_owned()) }, N)); }
                        alu::AluOp::Exp => { builder.add_child(Unary::new(val_receiver1, out_val_sender, |val: VT32| -> VT32 { Tensor::new(val.data.mapv(|x| x.exp()).to_owned()) }, N)); }
                        alu::AluOp::Rsqrt => { builder.add_child(Unary::new(val_receiver1, out_val_sender, |val: VT32| -> VT32 { Tensor::new(val.data.mapv(|x| 1.0 / x.sqrt()).to_owned()) }, N)); }
                        alu::AluOp::Sigmoid => { builder.add_child(Unary::new(val_receiver1, out_val_sender, |val: VT32| -> VT32 { Tensor::new(val.data.mapv(|x| 1.0 / (1.0 + (-x).exp())).to_owned()) }, N)); }
                        _ => todo!("Unsupported unary op for block sparse: {:?}", op.stages[0].op()),
                    }
                }
            }
...
pub fn build_from_proto_block64<'a>(
    ...
    for operation in comal_graph.graph.unwrap().operators {
        match operation.op.expect("Error processing") {
            ...
            Op::Joiner(op) => {
                // Use wildcard type Token<_, ST> to accept both CT (coordinates) and VT64 (Tensor blocks)
                let mut in_crds = Vec::new(); let mut in_refs: Vec<Box<dyn RecvAdapter<Token<_, ST>> + Send + Sync>> = Vec::new(); let mut out_refs: Vec<Box<dyn SendAdapter<Token<_, ST>> + Send + Sync>> = Vec::new();
            ...
            Op::Alu(op) => {
                ...
                if in_val_ids.len() == 2 {
                    ...
                    let (latency, ii, binary_func): (usize, usize, fn(VT64, VT64) -> VT64) = match op.stages[0].op() {
                        ...
                        alu::AluOp::Div => (N * N, 1, |val1: VT64, val2: VT64| -> VT64 { val1 / val2 }),
                        _ => todo!("Unsupported binary op for block sparse: {:?}", op.stages[0].op()),
                    };
                    builder.add_child(Binary::new(val_receiver1, val_receiver2, out_val_sender, binary_func, N.try_into().unwrap(), latency.try_into().unwrap(), ii.try_into().unwrap()));
                } else if in_val_ids.len() == 1 {
                    ...
                    match op.stages[0].op() {
                        ...
                        alu::AluOp::Scalardiv => { let scalar: f32 = op.scalar as f32; builder.add_child(Unary::new(val_receiver1, out_val_sender, move |val: VT64| -> VT64 { Tensor::new(val.data.map(|x| x / scalar).to_owned()) }, N)); }
                        alu::AluOp::Exp => { builder.add_child(Unary::new(val_receiver1, out_val_sender, |val: VT64| -> VT64 { Tensor::new(val.data.mapv(|x| x.exp()).to_owned()) }, N)); }
                        alu::AluOp::Rsqrt => { builder.add_child(Unary::new(val_receiver1, out_val_sender, |val: VT64| -> VT64 { Tensor::new(val.data.mapv(|x| 1.0 / x.sqrt()).to_owned()) }, N)); }
                        alu::AluOp::Sigmoid => { builder.add_child(Unary::new(val_receiver1, out_val_sender, |val: VT64| -> VT64 { Tensor::new(val.data.mapv(|x| 1.0 / (1.0 + (-x).exp())).to_owned()) }, N)); }
                        _ => todo!("Unsupported unary op for block sparse: {:?}", op.stages[0].op()),
                    }
                }
            }
