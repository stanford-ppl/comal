    ...
    StopType: DAMType + std::ops::Add<u32, Output = StopType> + std::cmp::PartialEq,
{
    ...
    fn run(&mut self) {
        ...
        let incr_type = data.sam_config.fiberlookup_incr_type;
        // dbg!(latency);
        // dbg!(initiation_interval);
            ...
            match self.rd_scan_data.in_ref.dequeue(&self.time) {
                Ok(curr_ref) => match curr_ref.data {
                    Token::Val(val) => {
                        ...
                        if idx - start_seg > 3 {
                            seg_initiated = true;
                            if incr_type == 2 || incr_type == 0 {
                                self.time
                                    .incr_cycles(data.sam_config.fiberlookup_miss_latency);
                            }
                        }
                        ...
                        while curr_addr < stop_addr {
                            ...
                            let mut final_latency = latency;
                            if read_addr - start_rd_addr > 3 {
                                initiated = true;
                                if incr_type == 2 || incr_type == 1 {
                                    final_latency = data.sam_config.fiberlookup_miss_latency;
                                }
                            }
                            ...
                            self.rd_scan_data
                                ...
                                .enqueue(
                                    ...
                                    ChannelElement::new(
                                        ...
                                        super::primitive::Token::Val(coord.clone()),
                                    ),
                                )
                                .unwrap();
                            self.rd_scan_data
                        ...
                        self.rd_scan_data
                            ...
                            .enqueue(
                                &self.time,
                                ChannelElement::new(curr_time + final_latency, output.clone()),
                            )
                            .unwrap();
                        self.rd_scan_data
                            .out_ref
                    ...
                    Token::Stop(token) => {
                        ...
                        self.rd_scan_data
                            ...
                            .unwrap();
                        // dbg!(Token::<ValType, StopType>::Stop(token.clone() + 1));
                    }
                    // Could either be a done token or an empty token
                    // In the case of done token, return
                    Token::Done => {
                    ...
                    _ => {} // Token::Empty => {
                            ...
                            //         .enqueue(&self.time, channel_elem.clone())
                            //         .unwrap();
                            //     dbg!(Token::<ValType, StopType>::Empty);
                            // }
                },
                Err(_) => panic!("Error: rd_scan_data dequeue error"),
            }
